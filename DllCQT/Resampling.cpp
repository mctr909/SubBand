...
HalfBandLowpass::HalfBandLowpass(int allpassNumber) {
...
bool HalfBandLowpass::init(const int expectedBlockSize, bool isDownsampling, bool isSampleBased, double transitionBandwidth) {
	...
	if (!isSampleBased) {
		...
		mDirectPathBuffer.resize(mFilterBufferSize, static_cast<BufferType>(0));
		mDelayPathBuffer.resize(mFilterBufferSize, static_cast<BufferType>(0));
		mOutputBlock.resize(mTargetBlockSize, static_cast<BufferType>(0));
	}
	return true;
}
...
BufferType* HalfBandLowpass::processBlockDown(BufferType* const inputBlock, const int inputBlockSize) {
	...
	for (int i = 0; i < mTargetBlockSize; i++) {
		mOutputBlock[i] = static_cast<BufferType>(0.5) * (mDirectPathBuffer[i] + mDelayPathBuffer[i]);
	}
